...
const Row = ({ children }: TableRowProps) => <tr className="hover:bg-indigo-100">{children}</tr>;
...
const FootData = ({ children, colspan, featured }: TableDataProps) => (
  <td colSpan={colspan} className={`py-2 px-4 border-t-2 border-indigo-300 font-bold ${featured ? 'border-r-2' : ''}`}>
    {children}
  </td>
);
...
const Table = ({ children }: TableProps) => {
  return (
    <div className="min-w-full border-2 border-indigo-300 rounded-lg">
      <table className="min-w-full overflow-x-auto rounded-lg">{children}</table>
    </div>
  );
};
